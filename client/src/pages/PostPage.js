...
export default function PostPage(){
    ...
    const {id} = useParams(); 
    useEffect(() => {
        fetch(`https://proj-api-fkij.onrender.com/post/${id}`)
        .then(response => {
            response.json()
            .then(content => {
    ...
    async function handleDelete() {
        await fetch(`https://proj-api-fkij.onrender.com/post/${id}`, {
            method: 'DELETE',
            credentials: 'include',
        }).then(() => {
            setRedirect(true)
    ...
    return (
        <div className="post-page">
            <div className="image">
                <img src={`https://proj-api-fkij.onrender.com/${postContent.cover}`} alt=""></img>
            </div>
